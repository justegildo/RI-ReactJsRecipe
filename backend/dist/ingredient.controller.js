...
module.exports.ingredientDetail = (req, res) => {
  ...
  IngredientModel.findById(req.params.id, (err, docs) => {
    if (!err) res.send(docs);else console.log('Id inconnu : ' + err);
  }).select();
};
...
module.exports.updateIngredient = async (req, res) => {
  ...
  try {
    ...
    IngredientModel.findByIdAndUpdate(req.params.id, {
      $set: updateRecord
    }, {
      new: true
    }, (err, data) => {
      if (!err) res.send(data);else console.log("Erreur de mise à jour : " + err);
    });
  } catch (err) {
...
module.exports.deleteIngredient = async (req, res) => {
  if (!ObjectID.isValid(req.params.id)) return res.status(400).send("ID inconnu : " + req.params.id);
  try {
    await IngredientModel.findByIdAndRemove({
      _id: req.params.id
    }).exec();
    res.status(200).json({
